...
public:
    ...
    explicit BranchHistoryTableMeasurer(Config config) : config_(std::move(config)) {
        SPDLOG_INFO("[{}] configured: min_period={}, max_period={}, coeff={}, iterations={}",
                    name(), config_.min_period, config_.max_period,
                    config_.period_coeff, config_.iterations);
    }
    ...
    void measure(core::CpuInfoData& data) override {
        ...
        for (size_t period = config_.min_period; period <= config_.max_period; period *= config_.period_coeff) {
            ...
            SPDLOG_INFO("[{}] period={:5d}  misses={:12}  miss_per_iter={:.4f}",
                        name(), period, misses, miss_per_iter);
        }
        ...
        if (bht_size < 0) {
            SPDLOG_ERROR("[{}] could not detect BHT saturation in period range [{}, {}]",
                         name(), config_.min_period, config_.max_period);
        } else {
...
private:
    ...
    int detectBHTSaturation(const std::vector<RawResult>& results) const {
        ...
        size_t baseline_cnt = std::min<size_t>(4, results.size() / 3);
        ...
        double thresh_baseline = baseline + 0.15;
        for (size_t i = 0; i < misses.size(); ++i) {
        ...
        if (max_delta_idx > 0 && max_delta > 0.05) {
        ...
        for (size_t i = 1; i < misses.size(); ++i) {
            if (misses[i-1] < 0.4 && misses[i] >= 0.4) {
                // Linear interpolation to find exact period at 0.4
                double t = (0.4 - misses[i-1]) / (misses[i] - misses[i-1]);
        ...
        SPDLOG_INFO("  baseline+0.15         → {} entries", est.threshold_baseline_plus_015);
        ...
        SPDLOG_INFO("  median cross at 0.4   → {} entries", est.median_cross);
