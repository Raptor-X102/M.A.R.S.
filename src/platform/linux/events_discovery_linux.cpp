...
std::vector<std::string> discover_port_events(shared_types::CpuInfoData& data) {
    ...
    if (data.cpu_vendor == CpuVendor::CpuVendorID::Intel) {
        ...
        for (int port = 0; port <= 7; ++port) {
            ...
            if (event_exists(name)) candidates.push_back(name);
        }

        // Load ports (grouped)
        std::vector<std::string> load_port_names = {"uops_dispatched.port_2_3_10", "uops_dispatched.port_2_3",
                                                    "uops_dispatched.port_2", "uops_dispatched.port_3"};
        ...
        // Store ports
        std::vector<std::string> store_port_names = {"uops_dispatched.port_4_9", "uops_dispatched.port_4",
                                                     "uops_dispatched.port_7_8", "uops_dispatched.port_7"};
...
std::optional<std::string> discover_branch_target_buffer_events(shared_types::CpuInfoData& data) {
    ...
    if (data.cpu_vendor == CpuVendor::CpuVendorID::Intel) {
        ...
        if (data.cpu_vendor)
            SPDLOG_WARN("Unsupported CPU vendor ({}) for branch target buffer events discovery",
...
std::vector<std::string> discover_s2l_forwarding_events(shared_types::CpuInfoData& data) {
    ...
    if (data.cpu_vendor == CpuVendor::CpuVendorID::Intel) {
        ...
        if (data.cpu_vendor)
            SPDLOG_WARN("Unsupported CPU vendor ({}) for store to load forwarding events discovery",
                        data.cpu_vendor->name());
    }
...
std::vector<std::string> discover_cache_miss_events(CacheLevel level, shared_types::CpuInfoData& data) {
    ...
    if (data.cpu_vendor == CpuVendor::CpuVendorID::Intel) {
        switch (level) {
            case CacheLevel::l1d:
                if (event_exists("mem_load_retired.l1_miss")) candidates.push_back("mem_load_retired.l1_miss");
                if (event_exists("l1-dcache-load-misses")) candidates.push_back("l1-dcache-load-misses");
                break;
            case CacheLevel::l2:
                if (event_exists("l2_rqsts.all_demand_miss")) candidates.push_back("l2_rqsts.all_demand_miss");
                if (event_exists("mem_load_retired.l2_miss")) candidates.push_back("mem_load_retired.l2_miss");
                break;
            case CacheLevel::l3:
                if (event_exists("longest_lat_cache.miss")) candidates.push_back("longest_lat_cache.miss");
                if (event_exists("mem_load_retired.l3_miss")) candidates.push_back("mem_load_retired.l3_miss");
                break;
            default:
