...
void BranchTargetBufferMeasurer::validateConfig() {
    ...
    if (config_.misprediction_saturation_threshold <= 0.0 || config_.misprediction_saturation_threshold > 1.0) {
        SPDLOG_WARN(
            "[{}] misprediction_saturation_threshold = {} is invalid, setting to 0.01", name(),
    ...
    if (config_.time_growth_ratio <= 1.0) {
        SPDLOG_WARN("[{}] time_growth_ratio = {} is invalid, setting to 1.2", name(), config_.time_growth_ratio);
...
std::optional<size_t> BranchTargetBufferMeasurer::findApproxSaturation(
    ...
    if (use_events) {
        ...
        size_t growth_idx = 0;
        if (results.size() <= config_.coarse_ignore_first)
            return std::nullopt;
        double prev_rate =
            computeMispredictionRate(results[config_.coarse_ignore_first - 1], counts[config_.coarse_ignore_first - 1]);
...
size_t BranchTargetBufferMeasurer::refineSaturationTime(size_t approx, double baseline, platform::pmc::PmcGroup* pmc) {
    if (baseline <= 0.0) {
