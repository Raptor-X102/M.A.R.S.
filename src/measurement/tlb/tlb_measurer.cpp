...
TlbMeasurer::Mapping::Mapping(Mapping&& other) noexcept { *this = std::move(other); }
...
std::string_view TlbMeasurer::name() const noexcept { return "tlb"; }
...
shared_types::TlbSummaryPoint TlbMeasurer::measure_point(
    ...
    if (config_.use_huge_pages) {
        ...
        if (mapping.base == nullptr) {
            throw platform::ResourceError(
                "Failed to allocate MAP_HUGETLB memory. "
                "Reserve huge pages or disable huge-page mode."
            );
        }
    } else {
...
std::vector<TlbMeasurer::PageNode*> TlbMeasurer::make_page_nodes(
...
void TlbMeasurer::warmup(PageNode* start, size_t pages) {
    ...
    const size_t accesses = std::max(pages, pages * kWarmupRounds);
...
std::optional<size_t> TlbMeasurer::find_latency_jump(
...
size_t TlbMeasurer::first_index_with_at_least_pages(
