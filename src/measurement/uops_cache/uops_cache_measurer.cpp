...
void UopsCacheMeasurer::measure(shared_types::CpuInfoData& data) {
    ...
    if (has_uops) {
        for (size_t i = 0; i < uops_events.size(); ++i) {
            if (uops_events[i].find("dsb_uops") != std::string::npos)
                dsb_idx = i;
            if (uops_events[i].find("mite_uops") != std::string::npos)
                mite_idx = i;
        }
    }
...
size_t UopsCacheMeasurer::findApproxSaturation(
    ...
    if (dsb_idx == std::string::npos || mite_idx == std::string::npos)
        return 0;

    std::vector<double> dsb_share;
    for (const auto& r : results) {
        if (r.avg_events_counts.empty())
            return 0;
